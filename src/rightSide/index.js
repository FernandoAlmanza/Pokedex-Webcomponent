...
class RightSide extends LitElement {
  static get properties() {
    return {
      ...
      ataque: { type: String },
      defensa: { type: String },
      especial1: { type: String },
      especial2: { type: String },
  ...
  __topScreen() {
    ...
    const aboutScreen = html`<section
      id="about-screen"
      class="right-panel-screen"
    >
      ${general} ${stats}
    </section>`;

    return html`<section class="top-screen-container">${aboutScreen}</section>`;
  }
  ...
  render = () => html`<aside id="right-panel">
    ${this.__blankContainer()} ${this.__topScreen()} ${this.__buttons()}
    ${this.__bottomScreen()}
  </aside>`;
}
